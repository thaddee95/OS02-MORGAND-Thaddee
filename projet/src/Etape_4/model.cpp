...
Model::Model( double t_length, int rank, int nbp, unsigned t_discretization, unsigned t_discretization_h, unsigned t_discretization_w, std::array<double,2> t_wind,
              LexicoIndices t_start_fire_position, double t_max_wind )
    :   m_length(t_length),
        m_distance(-1),
        m_geometry(t_discretization_w+2),
        m_wind(t_wind),
        m_wind_speed(std::sqrt(t_wind[0]*t_wind[0] + t_wind[1]*t_wind[1])),
        m_max_wind(t_max_wind),
        m_vegetation_map(t_discretization_h*(t_discretization_w+2), 255u),
        m_fire_map(t_discretization_h*(t_discretization_w+2), 0u)
{
    if ((t_discretization == 0)||(t_discretization_h == 0)||(t_discretization_w == 0))
    {
    ...
    unsigned start;
    if (rank < (int)t_discretization % nbp)
    {
        start = rank * t_discretization_w;
    }
    else {
        start = (t_discretization%nbp) * (t_discretization_w+1) + (rank - t_discretization%nbp) * t_discretization_w;
    }

    if ((t_start_fire_position.column>=start)&&(t_start_fire_position.column<start+t_discretization_w)){
        LexicoIndices t_start_fire_position_local=t_start_fire_position;
        t_start_fire_position_local.column = t_start_fire_position_local.column-start+1;
        auto index = get_index_from_lexicographic_indices(t_start_fire_position_local);
