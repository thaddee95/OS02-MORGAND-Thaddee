...
int main( int nargs, char* args[] )
{
    ...
    auto simu = Model( params.length, params.discretization, params.wind,
                       params.start);
    SDL_Event event;
    std::chrono::time_point < std::chrono::system_clock > start, end, start_display, end_display;
    std::chrono::duration < double >elapsed_seconds;
    std::chrono::time_point < std::chrono::system_clock > start_global, end_global;
    std::chrono::duration < double >elapsed_seconds_global;
    ...
    int count_display=0;
    /* Clé pour tester si les programmes renvoient le même résultat */
    unsigned key = 0;
    ...
    while (simu.update())
    {
        ...
        elapsed_seconds = end_display - start_display;
    ...
    elapsed_seconds_global = end_global - start_global;
    ...
    std::cout << " Average step time " << average_step.count() << " Average display time " << average_display.count() << std::endl;
    return EXIT_SUCCESS;
    return EXIT_SUCCESS;
}
